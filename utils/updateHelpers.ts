 ...
 */
export const setLastCheckTime = async (time: Date = new Date()): Promise<void> => {
  try {
    await AsyncStorage.setItem(UPDATE_CONFIG.STORAGE_KEYS.LAST_CHECK, time.toISOString());
...
export const getLastCheckTime = async (): Promise<Date | null> => {
  try {
    const timeString = await AsyncStorage.getItem(UPDATE_CONFIG.STORAGE_KEYS.LAST_CHECK);
    if (timeString) {
 ...
 * 사용자 업데이트 설정을 저장합니다
 */
export const saveUserSettings = async (settings: UpdateUserSettings): Promise<void> => {
  try {
...
export const getUserSettings = async (): Promise<UpdateUserSettings> => {
  try {
    const settingsString = await AsyncStorage.getItem(UPDATE_CONFIG.STORAGE_KEYS.USER_SETTINGS);
    if (settingsString) {
...
/**
 * 네트워크 연결 상태를 확인합니다
 * TODO: expo-network 패키지 추가 후 실제 네트워크 상태 확인 구현
 */
export const checkNetworkConnection = async (): Promise<{
  ...
  try {
    // 일단 연결되어 있다고 가정
    // 추후 expo-network 패키지 설치 후 실제 네트워크 상태 확인 구현
    return {
      isConnected: true,
      isWifi: true, // WiFi라고 가정
      connectionType: 'wifi',
    };
  } catch (error) {
 ...
 */
export const canDownloadUpdate = async (userSettings: UpdateUserSettings): Promise<boolean> => {
 ...
 */
export const getUpdateMessage = (metadata?: UpdateMetadata): {
  title: string;
  description: string;
  isForced: boolean;
} => {
  ...
  const { priority, forceUpdate = false, features = [], bugFixes = [] } = metadata;
  
  let title = UPDATE_CONFIG.MESSAGES.UPDATE_AVAILABLE;
  let description = UPDATE_CONFIG.MESSAGES.UPDATE_DESCRIPTION;
  ...
  switch (priority) {
    ...
    case 'high':
      ...
      if (features.length > 0) {
        description = `${features.slice(0, 2).join(', ')} 등의 새로운 기능을 만나보세요.`;
      }
      break;
    case 'medium':
      title = '앱이 더 좋아졌어요!';
