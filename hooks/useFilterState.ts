...
import { FilterState, initialFilterState } from '@/constants/filterConstants';

export const useFilterState = (initialState: FilterState = initialFilterState) => {
  const [filterState, setFilterState] = useState<FilterState>(initialState);

  // 추출 타입 설정
  const setBrewingType = useCallback((brewingType: FilterState['brewingType']) => {
