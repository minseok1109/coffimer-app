...
export const useUpdateNotification = (): UseUpdateNotificationReturn => {
  ...
  // 알림 타입 결정 로직
  const determineNotificationType = useCallback((metadata?: UpdateMetadata): NotificationType => {
    if (!metadata) {
  ...
  // 알림 표시 함수
  const showNotification = useCallback((type: NotificationType, metadata?: UpdateMetadata) => {
  ...
  }, [notification, isPending, isUpdateAvailable, downloadUpdate, applyUpdate, hideNotification]);
  ...
  }, [notification, isUpdateAvailable, isPending, downloadUpdate, hideNotification]);
  ...
  const onDismiss = useCallback(() => {
    ...
    dismissUpdate();
  }, [notification, updateInfo, dismissedUpdates, dismissUpdate, hideNotification]);
  ...
  useEffect(() => {
    if (status === 'available' && shouldShowNotification && !notification.show) {
      const metadata: UpdateMetadata = {
  ...
  }, [status, shouldShowNotification, notification.show, determineNotificationType, showNotification]);
